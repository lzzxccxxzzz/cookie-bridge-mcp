...
// Cookie Clicker 2.053 bundles an older Node without crypto.randomUUID.
...
function createQueue({ maxPending = 1000, maxHistory = 200, storagePath, now = Date.now, dispatchTimeoutMs = 120000 } = {}) {
  const pending = [], records = new Map(), order = [];
  if (storagePath && fs.existsSync(storagePath)) {
    const saved = JSON.parse(fs.readFileSync(storagePath, 'utf8'));
    if (saved.version !== 1 || !Array.isArray(saved.records) || !Array.isArray(saved.pending)) throw new Error('Invalid action journal');
    for (const record of saved.records) {
      if (record.status === 'dispatched') Object.assign(record, {status: 'indeterminate', error: {code: 'bridge_restarted', message: 'The bridge restarted after dispatch. Inspect game state before retrying.', may_have_side_effects: true}});
      records.set(record.id, record); order.push(record.id);
    }
    pending.push(...saved.pending);
  }
  function persist() {
    if (!storagePath) return;
    fs.mkdirSync(path.dirname(storagePath), {recursive: true});
    fs.writeFileSync(storagePath + '.tmp', JSON.stringify({version: 1, records: order.map(id => records.get(id)), pending}));
    fs.renameSync(storagePath + '.tmp', storagePath);
  }
  function sweep() {
    let changed = false;
    for (let i = pending.length - 1; i >= 0; i--) {
      const r = records.get(pending[i]._bridge.id);
      if (now() > r.expires_at) { r.status = 'expired'; r.finished_at = now(); pending.splice(i, 1); changed = true; }
    }
    for (const r of records.values()) if (r.status === 'dispatched' && now() - r.dispatched_at > dispatchTimeoutMs) {
      Object.assign(r, {status: 'indeterminate', error: {code: 'receipt_timeout', message: 'Execution receipt was lost or delayed. Inspect game state; do not repeat the action automatically.', may_have_side_effects: true}}); changed = true;
    }
  ...
  function enqueue(input, options = {}) {
    ...
    const id = randomUUID(), queuedAt = now();
    ...
    records.set(id, record); order.push(id); pending.push(envelope); trim(); persist();
  ...
  function next() {
    ...
    while (pending.length) {
      ...
      record.status = 'dispatched'; record.dispatched_at = now(); persist(); return action;
    }
    trim(); return null;
  }
  function acknowledge(receipts) {
    ...
    receipts.forEach(receipt => {
      ...
      if (!record || !['dispatched', 'indeterminate'].includes(record.status)) return; // idempotent receipt replay
    ...
    trim(); persist(); return { ok: true, acknowledged: receipts.map(r => r.id), orphaned };
  }
  function get(id) { sweep(); return records.get(id) || null; }
  function history(count) { sweep(); return order.slice(-count).map(id => records.get(id)); }
  function clear() {
    const cancelled = pending.splice(0).map(action => action._bridge.id);
    cancelled.forEach(id => Object.assign(records.get(id), { status: 'cancelled', finished_at: now() }));
    trim(); persist(); return { ok: true, cancelled, count: cancelled.length };
  }
  sweep(); trim(); persist();
  return {enqueue, next, acknowledge, get, history, clear, view: () => { sweep(); return {total: pending.length, fila: pending.map(a => ({...a, save: a.save ? '[redacted]' : undefined, code: a.code ? '[redacted]' : undefined}))}; }};
}
