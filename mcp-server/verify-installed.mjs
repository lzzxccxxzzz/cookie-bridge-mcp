...
await until(async()=>{const c=await json(url+'/capabilities');return c.renderer_version===schema.version && c.state_age_ms<3000;},30000);
...
try {
  ...
  for(const language of ['','/pt']) {const response=await fetch(url+'/docs'+language);assert.equal(response.status,200);assert.ok((await response.text()).includes('v'+schema.version));}
